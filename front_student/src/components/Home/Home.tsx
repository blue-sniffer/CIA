...
const Home: React.FC = () => {
  ...
  const totalPrice: number = products.products.reduce((prev, next) => prev + ((Number(next.price) * Number(next.amount)) || 0), 0);
  const totalProductAmount: number = products.products.reduce((prev, next) => prev + (Number(next.amount) || 0), 0);

  const orders: IOrder[] = useSelector((state: IStateType) => state.orders.orders);
  const totalSales: number = orders.reduce((prev, next) => prev + Number(next.totalPrice), 0);
  const totalOrderAmount: number = orders.reduce((prev, next) => prev + Number(next.amount), 0);
  ...
  return (
    <Fragment>
      ...
      <div className="row">
        <TopCard title="SALES" text={`$${totalSales}`} icon="donate" class="primary" />
